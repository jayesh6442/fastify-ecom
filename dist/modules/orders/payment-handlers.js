...
import { createRazorpayOrder, verifyRazorpayPaymentSignature, verifyRazorpayWebhookSignature, RAZORPAY_KEY_ID } from '../../services/payment.js';
...
export async function createPaymentHandler(request, reply) {
    ...
    try {
        ...
        const razorpayOrder = await createRazorpayOrder(amountPaise, `order_${orderId}`, { order_id: String(orderId), user_id: String(user.id) });
        if (!razorpayOrder) {
...
export async function confirmPaymentHandler(request, reply) {
    ...
    try {
        ...
        const result = await updateOrderStatus(request.server.db, orderId, 'PAID', 'CREATED', { order_id: razorpay_order_id, payment_id: razorpay_payment_id });
...
export async function razorpayWebhookHandler(request, reply) {
    ...
    try {
        ...
        await updateOrderStatus(request.server.db, ourOrder.id, 'PAID', 'CREATED', { order_id: razorpayOrderId, payment_id: razorpayPaymentId });
